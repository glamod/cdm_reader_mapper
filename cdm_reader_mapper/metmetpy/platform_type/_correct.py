...
from cdm_reader_mapper.common.local import get_file

from .. import properties  # noqa
from . import gdac_r0000  # noqa
from . import icoads_r3000  # noqa
from . import icoads_r3000_NRT  # noqa
from .correction_functions import fill_value

_base = f"{properties._base}.platform_type"
_files = get_file(_base)
...
def correct_it(data, dataset, data_model, deck, pt_col, fix_methods, log_level="INFO"):
    ...
    if deck_fix.get("method") == "fillna":
        ...
        trans = getattr(dataset, transform)
        return trans(data)
    else:
        logger.error(

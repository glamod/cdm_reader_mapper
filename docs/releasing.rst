...
.. warning::
    ...
    It is important to be aware that any changes to files found within the ``src/cdm_reader_mapper`` folder (with the exception of ``src/cdm_reader_mapper/__init__.py``) will trigger the ``bump-version.yml`` workflow. Be careful not to commit changes to files in this folder when preparing a new release.
...
From the cdm_reader_mapper source folder we can enter into the docker container, providing access to the `src/cdm_reader_mapper` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/cdm_reader_mapper -w /src/cdm_reader_mapper quay.io/pypa/manylinux_2_24_x86_64 bash
...
This will then place two files in `cdm_reader_mapper/dist/` ("cdm_reader_mapper-1.2.3-py3-none-any.whl" and "cdm_reader_mapper-1.2.3.tar.gz").

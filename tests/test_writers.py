...
db_exp = read(
    inp_dir=dict(expected_data)["cdm_table"], suffix=f"{imodel}*", mode="tables"
)


def test_write_data():
    db_exp.write(suffix=f"{imodel}_all")
    db_res = read(inp_dir=".", suffix=f"{imodel}_all", mode="tables")
...
def test_write_tables(table):
    ...
    db_res = read(
        inp_dir=".", suffix=f"{imodel}_{table}_all", cdm_subset=table, mode="tables"
    )
...
def test_write_filename():
    ...
    db_res = read(inp_dir=".", suffix=f"{imodel}_filename_all", mode="tables")
    pd.testing.assert_frame_equal(db_exp.data, db_res.data)
...
def test_write_filename_dict():
    ...
    db_res = read(inp_dir=".", suffix=f"{imodel}_filename_dict_all", mode="tables")
    tables_exp = db_exp.data.copy()
...
def test_write_col_subset():
    ...
    db_res = read(inp_dir=".", suffix=f"{imodel}_{table}_all", mode="tables")
    tables_exp = db_exp.data.copy()

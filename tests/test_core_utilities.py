...
def test_combine_attribute_values_index():
    first = pd.Index([1, 2])
    iterator = [SimpleNamespace(attr=pd.Index([2, 3])), SimpleNamespace(attr=pd.Index([3, 4]))]
...
def test_combine_attribute_values_ndarray():
    first = np.array([1, 2])
    iterator = [SimpleNamespace(attr=np.array([3, 4])), SimpleNamespace(attr=np.array([5]))]
...
def test_combine_attribute_values_pandas():
    ...
    iterator = [SimpleNamespace(attr=pd.Series([3, 4], index=[2,3])), SimpleNamespace(attr=pd.Series([5], index=[4]))]
...
def test_subscriptablemethod_getitem_typeerror():
    ...
    with pytest.raises(NotImplementedError, match="Calling subscriptable methods have not been implemented"):
        sm[0]      
...
def test_stack_inplace(test_db_tuple):
    ...
    expected_df = pd.concat([pd.DataFrame({"a": [1,2], "b": [3,4]}),
